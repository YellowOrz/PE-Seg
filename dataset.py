...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = cv2.imread(os.path.join(self.mask_dir, img_id.replace(".jpg", "_segmentation.png")), flags=0)
...
#         mask = cv2.imread(os.path.join(self.mask_dir, img_id), flags=cv2.IMREAD_GRAYSCALE)
#         mask = np.expand_dims(mask, axis=-1)
#         mask = np.concatenate([mask, mask], axis=-1)
#
#         if len(img.shape)==2:
